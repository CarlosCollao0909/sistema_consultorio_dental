<h1 class="mb-6 text-3xl font-bold tracking-tight text-slate-800">Registrar Nueva Cita</h1>
...
<div class="rounded-xl border border-slate-200 bg-white p-6 shadow-md md:p-8">
    ...
    <form action="/admin/appointments/create?patient_id=<?php echo $patientId; ?>&treatment_id=<?php echo $treatment->id; ?>" method="POST" class="space-y-6">
        <input type="hidden" name="patient_id" value="<?php echo $patientId; ?>">
        <input type="hidden" name="treatment_id" value="<?php echo $treatment->id; ?>">
        <?php include_once 'form.php'; ?>
        <input type="submit" value="Registrar Cita" class="w-full cursor-pointer rounded-lg bg-emerald-600 px-6 py-3 font-semibold text-white transition hover:bg-emerald-700">
    </form>
</div>
